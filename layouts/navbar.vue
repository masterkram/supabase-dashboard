  ...
  <div class="min-h-full">
        ...
        <div class="flex h-16 items-center justify-between">
          <div class="flex items-center">
            <div class="flex-shrink-0">
              <Icon class="h-8 w-8 text-emerald-500" name="carbon:tsunami" />
            </div>
            <div class="hidden md:block">
              <div class="ml-10 flex items-baseline space-x-4">
                <NuxtLink v-for="item in navigation" :key="item.name" :to="item.href"
                  :class="[item.current ? 'bg-zinc-900 text-white' : 'text-zinc-300 hover:bg-zinc-700 hover:text-white', 'rounded-md px-3 py-2 text-sm font-medium']"
          ...
          <div class="hidden md:block">
            <div class="ml-4 flex items-center md:ml-6">
              ...
              <Menu as="div" class="relative ml-3">
                <div>
                  <MenuButton
                    class="relative flex max-w-xs items-center rounded-full bg-zinc-800 text-sm text-white focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-zinc-800">
                    <span class="absolute -inset-1.5" />
                ...
                <transition enter-active-class="transition ease-out duration-100"
                  ...
                  <MenuItems
                    ...
                    <a :href="item.href"
                      :class="[active ? 'bg-zinc-100' : '', 'block px-4 py-2 text-sm text-zinc-700']">{{ item.name
                      }}</a>
                    </MenuItem>
                  </MenuItems>
                </transition>
      ...
      <DisclosurePanel class="md:hidden">
        <div class="space-y-1 px-2 pb-3 pt-2 sm:px-3">
          <DisclosureButton v-for="item in navigation" :key="item.name" as="a" :href="item.href"
            :class="[item.current ? 'bg-zinc-900 text-white' : 'text-zinc-300 hover:bg-zinc-700 hover:text-white', 'block rounded-md px-3 py-2 text-base font-medium']"
        ...
        <div class="border-t border-zinc-700 pb-3 pt-4">
          <div class="flex items-center px-5">
            ...
            <div class="ml-3">
              ...
              <div class="text-sm font-medium text-zinc-400">{{ user.email }}</div>
            </div>
            <button type="button"
    ...
    <header class="bg-white shadow-sm">
      <div class="mx-auto max-w-7xl px-4 py-4 sm:px-6 lg:px-8">
        <h1 class="text-lg font-semibold leading-6 text-zinc-900">Dashboard</h1>
      </div>
    </header>
    <main>
